...
async function install(callback) {
    ...
    const installPath = await getInstallationPath(opts);
    if (process.platform === "win32") {
        ...
        await execShellCommand(`chmod +x "${installPath}/${opts.binName}"`);
    }
...
async function uninstall(callback) {
    var opts = parsePackageJson(".");
    try {
        const installationPath = await getInstallationPath(opts);
