...
namespace Cmf.CLI.Commands
{
    ...
    internal class InitArguments
    {
        ...
        public string[] releaseRepos { get; set; }
    ...
    public class InitCommand : TemplateCommand
    {
        ...
        internal void Execute(InitArguments x)
        {
            ...
            Log.Debug($"Deployment directory: {x.deploymentDir?.FullName ?? "null"}");
            Log.Debug($"CI repository: {x.ciRepo ?? "null"}");
            Log.Debug($"Release repository: {(x.releaseRepos != null ? string.Join(";", x.releaseRepos) : "null")}");

            throw new PluginException("Testing");
            ...
            else
            {
                if (x.ciRepo == null)
                {
                    throw new CliException("Missing option `--ciRepo` or `--deploymentDir`. Please specify one or the other (but not none).");
                }
                if (x.releaseRepos == null)
                {
                    throw new CliException("Missing option `--releaseRepos` or `--deploymentDir`. Please specify one or the other (but not none).");
                }

                args.AddRange(new[] { "--CIRepo", x.ciRepo });
                args.AddRange(new[] { "--ReleaseRepos", string.Join(";", x.releaseRepos) });
            }
